...
// for PS3.. non static, extern
int set_relative_speed(int val, void *param)
{
    relative_speed = val;
...
/*
#ifdef PS3
    // If we've slowed down, try throttling back some features.
    if ( (signed long)(now - next_frame_start) >= vsyncarch_freq / 16) {
        int passband_percentage;
        int resid_sampling;
        int sid_engine;
        int sid_model;

        // Filters no longer auto-disable. Performance is good enough that it isn't necessary.
        // Leaving it up to the user to decide.  Even the worst filter still achieves 20fps at Max ReSID-FP

//        if (shader_active()) {
//            log_warning(LOG_DEFAULT, "Disabling shaders");
//            disable_active_shader();
//        }
//        else 

        if (resources_get_int("SidResidPassband", &passband_percentage) == 0) {

            if (passband_percentage >= 20) {
            	passband_percentage -= 10;
            	log_warning(LOG_DEFAULT, "SidResidPassband automatically reduced to %d", passband_percentage);
            	resources_set_int("SidResidPassband", passband_percentage);
            } else {
                if (resources_get_int("SidResidSampling", &resid_sampling) == 0) {
                    if (resid_sampling == 2) {
                        resid_sampling = 1;
                        log_warning(LOG_DEFAULT, "SidFilter resampling automatically demoted to interpolation");
                        resources_set_int("SidResidSampling", resid_sampling);
                    } else if (resid_sampling == 1) {
                        resid_sampling = 0;
                        log_warning(LOG_DEFAULT, "SidFilter interpolation automatically demoted to FastSID");
                        resources_set_int("SidResidSampling", resid_sampling);
                    }
                    else {
                        if ( (resources_get_int("SidEngine", &sid_engine) == 0) && (resources_get_int("SidModel", &sid_model) == 0 ) ) {
                            if (sid_engine == SID_ENGINE_RESID_FP) {
                                log_warning(LOG_DEFAULT, "SidEngine automatically demoted to ReSid (model %d)", sid_model);
                        	resources_set_int("SidEngine", SID_ENGINE_RESID);
                                sid_set_engine_model(SID_ENGINE_RESID, sid_model);
                            }
                            else if (sid_engine == SID_ENGINE_RESID) {
                                log_warning(LOG_DEFAULT, "SidEngine automatically demoted to FastSid (model %d)", sid_model);
                        	resources_set_int("SidEngine", SID_ENGINE_FASTSID);
                                sid_set_engine_model(SID_ENGINE_FASTSID, sid_model);
                            }
                        }
                    }
                }
