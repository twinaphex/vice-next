...
//
//  Updated by TimRex for Altivec routines (December 2010)
// 
...
extern float convolve_altivec(const float *a, const float *b, int n);

enum host_cpu_feature {
    HOST_CPU_MMX=1, HOST_CPU_SSE=2, HOST_CPU_SSE2=4, HOST_CPU_SSE3=8, HOST_CPU_ALTIVEC=16
};
...
SIDFP::SIDFP()
{
  ...
  can_use_altivec = false;
#elif RESID_USE_ALTIVEC==1
  can_use_altivec = true;    // hardcoded. If we've said we can use altivec, believe it.
  can_use_sse = false;
#else
  can_use_sse = false;
  can_use_altivec = false;
...
#elif (RESID_USE_ALTIVEC==1)
      can_use_altivec ? convolve_altivec(sample_start, fir + fir_offset*fir_N, fir_N) :
#endif
