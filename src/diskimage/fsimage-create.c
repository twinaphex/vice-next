...
static int fsimage_create_gcr(disk_image_t *image)
{
...
#ifdef CELL_DEBUG
		printf("ERROR: Cannot write GCR header.\n");
#endif
		return -1;
	}
	...
	{
		#ifdef CELL_DEBUG
		printf("ERROR: Cannot write track header.\n");
		#endif
		return -1;
	}
	...
	{
		#ifdef CELL_DEBUG
		printf("ERROR: Cannot write speed header.\n");
		#endif
		return -1;
	}
	for (track = 0; track < MAX_TRACKS_1541; track++)
	{
		...
		{
			#ifdef CELL_DEBUG
			printf("ERROR: Cannot write track data.\n");
			#endif
			return -1;
		}
	}
...
int fsimage_create(const char *name, unsigned int type)
{
	...
	switch(type)
	{
		...
		default:
			#ifdef CELL_DEBUG
			printf("ERROR: Wrong image type.  Cannot create disk image.\n");
			#endif
			return -1;
	}
	...
	{
		#ifdef CELL_DEBUG
		printf("ERROR: Cannot create disk image `%s'.\n", fsimage->name);
		#endif
	...
	switch (type) {
		case DISK_IMAGE_TYPE_X64:
			{
				...
				{
					#ifdef CELL_DEBUG
					printf("Cannot write X64 header to disk image `%s'.\n", fsimage->name);
					#endif
				}
		...
		case DISK_IMAGE_TYPE_D82:
			for (i = 0; i < (size / 256); i++)
			{
				...
				{
					#ifdef CELL_DEBUG
					printf("Cannot seek to end of disk image `%s'.\n", fsimage->name);
					#endif
					rc = -1;
					break;
				}
