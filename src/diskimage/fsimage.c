                         ...
                         unsigned int sector)
{
	...
	{
		#ifdef CELL_DEBUG
		printf("ERROR: Attempt to write without disk image.\n");
		#endif
		return -1;
	}
	...
	{
		#ifdef CELL_DEBUG
		printf("ERROR: Attempt to write to read-only disk image.\n");
		#endif
		return -1;
	}
	...
	switch (image->type)
	{
		...
		case DISK_IMAGE_TYPE_X64:
			...
			{
				#ifdef CELL_DEBUG
				printf("ERROR: Error writing T:%i S:%i to disk image.\n", track, sector);
				#endif
				return -1;
			}
		...
		default:
			#ifdef CELL_DEBUG
			printf("ERROR: Unknown disk image.  Cannot write sector.\n");
			#endif
			return -1;
	}
	return 0;
