...
static int plus256k_activate(void)
{
	...
	#ifdef CELL_DEBUG
	printf("INFO: PLUS256K hack installed.\n");
	#endif
	...
	if (!util_check_null_string(plus256k_filename))
	{
		...
		{
			#ifdef CELL_DEBUG
			printf("INFO: Reading PLUS256K image %s failed.\n", plus256k_filename);
			#endif
			if (util_file_save(plus256k_filename, plus256k_ram, 0x40000) < 0)
			{
				#ifdef CELL_DEBUG
				printf("INFO: Creating PLUS256K image %s failed.\n", plus256k_filename);
				#endif
				return -1;
			}
			#ifdef CELL_DEBUG
			printf("INFO: Creating PLUS256K image %s.\n", plus256k_filename);
			#endif
			return 0;
		}
		#ifdef CELL_DEBUG
		printf("INFO: Reading PLUS256K image %s.\n", plus256k_filename);
		#endif
	}
	plus256k_reset();
...
static int plus256k_deactivate(void)
{
	if (!util_check_null_string(plus256k_filename))
	{
		...
		{
			#ifdef CELL_DEBUG
			printf("INFO: Writing PLUS256K image %s failed.\n", plus256k_filename);
			#endif
			return -1;
		}
		#ifdef CELL_DEBUG
		printf("INFO: Writing PLUS256K image %s.\n", plus256k_filename);
		#endif
	}
