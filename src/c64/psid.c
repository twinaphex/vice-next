...
int psid_load_file(const char* filename)
{
	...
	{
		#ifdef CELL_DEBUG
		printf("ERROR: Unknown PSID version number: %d.\n", (int)psid->version);
		#endif
		goto fail;
	}
	...
	{
		printf("ERROR: Reading PSID header.\n");
		goto fail;
	}
	...
	if (psid->flags & 0x01)
	{
		printf("ERROR: SIDPLAYER MUS files not supported.\n");
		goto fail;
	}
	...
	if (psid->load_addr == 0)
	{
		...
		{
			printf("ERROR: Reading PSID load address.\n");
			goto fail;
		}
	...
	if (ferror(f))
	{
		printf("ERROR: Reading PSID data.\n");
		goto fail;
	}

	if (!feof(f))
	{
		printf("ERROR: More than 64K PSID data.\n");
		goto fail;
	}
	...
	{
		#ifdef CELL_DEBUG
		printf("ERROR: No space for driver.\n");
		#endif
		goto fail;
	}
...
void psid_init_tune(void)
{
	...
	#ifdef CELL_DEBUG
	printf("INFO: Driver=$%04X, Image=$%04X-$%04X, Init=$%04X, Play=$%04X\n", reloc_addr, psid->load_addr, psid->load_addr + psid->data_size - 1, psid->init_addr, psid->play_addr);
	#endif
	...
	{
		#ifdef CELL_DEBUG
		printf("WARNING: Tune out of range.\n");
		#endif
		start_song = psid->start_song;
	}
	...
	{
		#ifdef CELL_DEBUG
		printf("WARNING: Image is PlaySID specific - trying anyway.\n");
		#endif
	}
	...
	if (console_mode)
	{
		...
		printf("SID model: %s  (Using %s)\n", csidflag[(psid->flags >> 4) & 3], csidmodel[sid_model > (int)MAX_SIDMODEL ? DEFAULT_SIDMODEL : sid_model]);
		printf("Using %s interrupt\n", irq_str);
		printf("Playing tune %d out of %d (default=%d)\n", start_song, psid->songs, psid->start_song);
		#endif
	}
	else
	{
