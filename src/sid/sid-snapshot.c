...
static int sid_snapshot_read_module_simple(snapshot_t *s)
{
	...
	{
		#ifdef CELL_DEBUG
		printf("ERROR: SID: Snapshot module version (%d.%d) newer than %d.%d.\n\n", major_version, minor_version, SNAP_MAJOR_SIMPLE, SNAP_MINOR_SIMPLE);
		#endif
		snapshot_module_close(m);
		goto fail;
	}
...
fail:
	#ifdef CELL_DEBUG
	printf("ERROR: Failed reading SID snapshot\n");
	#endif
	return -1;
}
...
static int sid_snapshot_read_module_extended(snapshot_t *s)
{
	...
	{
		printf("ERROR: SID: Snapshot module version (%d.%d) newer than %d.%d.\n", major_version, minor_version, SNAP_MAJOR_EXTENDED, SNAP_MINOR_EXTENDED);
		return snapshot_module_close(m);
	}
