...
static int residfp_init(sound_t *psid, int speed, int cycles_per_sec)
{
	...
	if (!psid->sid->set_sampling_parameters((float)cycles_per_sec, method, (float)speed, passband))
	{
#ifdef CELL_DEBUG
		printf("WARNING: ReSID-FP: unable to set sampling mode; try increasing sampling frequency to 44.1-48 kHz and keep passband around 80-90 %%.\n");
#endif
		return 0;
	}

#ifdef CELL_DEBUG
	printf("INFO: ReSID-FP: %s, filter %s, sampling rate %d Hz with %s\n", model_text, filters_enabled ? "on" : "off", speed, method_text);
#endif
	return 1;
}
...
static int residfp_calculate_samples(sound_t *psid, SWORD *pbuf, int nr, int interleave, int *delta_t)
{
