...
static int set_system_path(const char *val, void *param)
{
    ...
    do {
        ...
        if (!archdep_path_is_relative(tmp_path)) {
            /* absolute path */
            if ((expanded_system_path == NULL) || (strlen(expanded_system_path) == 0)){
            ...
            if (expanded_system_path == NULL || (strlen(expanded_system_path) == 0)) {
                s = util_concat(tmp_path, NULL );
            } else {
                s = util_concat( expanded_system_path, ARCHDEP_FINDPATH_SEPARATOR_STRING, tmp_path, NULL );
            }
        }
        lib_free(expanded_system_path);
