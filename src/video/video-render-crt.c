...
static void video_render_crt_main(video_render_config_t *config, BYTE *src, BYTE *trg, int width, int height, int xs, int ys, int xt, int yt, int pitchs, int pitcht, int depth, viewport_t *viewport)
{
	video_render_color_tables_t *colortab;
	int doublescan, delayloop, rendermode, video;
	...
	if ((rendermode == VIDEO_RENDER_CRT_1X1 || rendermode == VIDEO_RENDER_CRT_1X2 || rendermode == VIDEO_RENDER_CRT_2X2) && video_resources.pal_scanlineshade <= 0)
		doublescan = 0;
