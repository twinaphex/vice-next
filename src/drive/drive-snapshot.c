...
int drive_snapshot_write_module(snapshot_t *s, int save_disks, int save_roms)
{
	...
	{
		drive = drive_context[i]->drive;
		if (0 || SMW_DW(m, (DWORD)(drive->attach_detach_clk)) < 0)
		{
			if (m != NULL)
				snapshot_module_close(m);
			return -1;
	...
	{
		drive = drive_context[i]->drive;
		if (0 || SMW_B(m, (BYTE)(drive->byte_ready_edge)) < 0 || SMW_B(m, (BYTE)(drive->byte_ready_active)) < 0)
		{
			if (m != NULL)
				snapshot_module_close(m);
			return -1;
...
static int drive_snapshot_read_image_module(snapshot_t *s, unsigned int dnr)
{
    ...
    {
    	#ifdef CELL_DEBUG
    	printf("ERROR: Snapshot module version (%d.%d) newer than %d.%d.\n", major_version, minor_version, IMAGE_SNAP_MAJOR, IMAGE_SNAP_MINOR);
	#endif
    }
    ...
    switch(word)
    {
	    ...
	    default:
#ifdef CELL_DEBUG
		    printf("ERROR: Snapshot of disk image unknown (type %d)\n", (int)word);
#endif
		    snapshot_module_close(m);
		    return -1;
    }
    ...
    if (fseek(fp, len - 1, SEEK_SET) < 0 || (fputc(0, fp) == EOF))
    {
#ifdef CELL_DEBUG
	    printf("ERROR: Could not create large temporary file\n");
#endif
	    fclose(fp);
	    lib_free(filename);
	    snapshot_module_close(m);
    ...
    {
#ifdef CELL_DEBUG
	    printf("ERROR: Invalid Disk Image\n");
#endif
	    lib_free(filename);
	    snapshot_module_close(m);
	    return -1;
...
static int drive_snapshot_read_gcrimage_module(snapshot_t *s, unsigned int dnr)
{
	...
	{
#ifdef CELL_DEBUG
		printf("ERROR: Snapshot module version (%d.%d) newer than %d.%d.\n", major_version, minor_version, GCRIMAGE_SNAP_MAJOR, GCRIMAGE_SNAP_MINOR);
#endif
	}
...
static int drive_snapshot_read_rom_module(snapshot_t *s, unsigned int dnr)
{
	...
	{
		#ifdef CELL_DEBUG
		printf("ERROR: Snapshot module version (%d.%d) newer than %d.%d.\n", major_version, minor_version, ROM_SNAP_MAJOR, ROM_SNAP_MINOR);
		#endif
	}
