...
static void REGPARM3 drive_store_watch(drive_context_t *drv, WORD address, BYTE value)
{
...
void drivemem_set_func(drivecpud_context_t *cpud, unsigned int start, unsigned int stop, drive_read_func_t *read_func, drive_store_func_t *store_func)
{
	unsigned int i;
...
void drivemem_init(drive_context_t *drv, unsigned int type)
{
	...
	switch (type)
	{
		...
		default:
#ifdef CELL_DEBUG
			printf("ERROR: DRIVEMEM: Unknown drive type `%i'.\n", type);
#endif
			break;
	}
}
