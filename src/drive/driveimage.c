...
inline static unsigned int sector_offset(unsigned int track, unsigned int sector, unsigned int max_sector, drive_t *drive)
{
	return GCR_OFFSET(track) + (SECTOR_GCR_SIZE_WITH_HEADER + gaps_between_sectors[disk_image_speed_map_1541(track-1)]) * sector;
}
...
void drive_image_init_track_size_d64(drive_t *drive)
{
    ...
    {
	    drive->gcr->track_size[track] = raw_track_size[disk_image_speed_map_1541(track)];
	    memset(drive->gcr->speed_zone, disk_image_speed_map_1541(track), NUM_MAX_BYTES_TRACK);
    }
}
...
static void drive_image_init_track_size_d71(drive_t *drive)
{
    ...
    {
	    drive->gcr->track_size[track] = raw_track_size[disk_image_speed_map_1571(track)];
	    memset(drive->gcr->speed_zone, disk_image_speed_map_1571(track), NUM_MAX_BYTES_TRACK);
    }
}
