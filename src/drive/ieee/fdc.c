...
void fdc_reset(unsigned int fnum, unsigned int drive_type)
{
	...
	if (fnum == 0 && drive_check_dual(drive_type) && fdc[1].realimage)
	...
	if (!memcmp(fdc[fnum].iprom + 0x2040, &fdc[fnum].buffer[0x100], 0x200)) {
		...
		for (ret = 0, track = 1; ret == 0 && track <= ntracks; track ++)
		{
			...
			for (sector = 0; sector < nsectors; sector ++)
			{
				...
				{
#ifdef CELL_DEBUG
					printf("ERROR: Could not update T:%d S:%d on disk image.\n", track, sector);
#endif
					rc = FDC_ERR_DCHECK;
					break;
				}
...
#endif
			if (DOS_IS_40(fdc[fnum].drive_type) || DOS_IS_30(fdc[fnum].drive_type))
			{
...
int fdc_detach_image(disk_image_t *image, unsigned int unit)
{
	...
	if (fdc[drive_no].drive_type == DRIVE_TYPE_8050 || fdc[drive_no].drive_type == DRIVE_TYPE_8250 || fdc[drive_no].drive_type == DRIVE_TYPE_1001)
	{
		switch(image->type)
		{
			case DISK_IMAGE_TYPE_D80:
...
int fdc_snapshot_read_module(snapshot_t *p, int fnum)
{
	...
	{
		#ifdef CELL_DEBUG
		printf("INFO: Could not find snapshot module %s\n", name);
		#endif
		return -1;
	}
