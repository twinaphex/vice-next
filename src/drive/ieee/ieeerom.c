...
int ieeerom_load_2031(void)
{
	...
	if (sysfile_load(rom_name, drive_rom2031, DRIVE_ROM2031_SIZE, DRIVE_ROM2031_SIZE) < 0)
	{
		#ifdef CELL_DEBUG
		printf("ERROR: 2031 ROM image not found.\nHardware-level 2031 emulation is not available.\n");
		#endif
	} else {
		rom2031_loaded = 1;
...
int ieeerom_load_2040(void)
{
	...
	if (sysfile_load(rom_name, drive_rom2040, DRIVE_ROM2040_SIZE, DRIVE_ROM2040_SIZE) < 0)
	{
		#ifdef CELL_DEBUG
		printf("2040 ROM image not found.\nHardware-level 2040 emulation is not available.\n");
		#endif
	}
	else
	{
...
int ieeerom_load_3040(void)
{
	...
	if (sysfile_load(rom_name, drive_rom3040, DRIVE_ROM3040_SIZE, DRIVE_ROM3040_SIZE) < 0)
	{
		#ifdef CELL_DEBUG
		printf("3040 ROM image not found.\nHardware-level 3040 emulation is not available.\n");
		#endif
	}
	else
	{
...
int ieeerom_load_4040(void)
{
	...
	if (sysfile_load(rom_name, drive_rom4040, DRIVE_ROM4040_SIZE, DRIVE_ROM4040_SIZE) < 0)
	{
		#ifdef CELL_DEBUG
		printf("4040 ROM image not found.\nHardware-level 4040 emulation is not available.\n");
		#endif
	}
	else
	{
...
int ieeerom_load_1001(void)
{
	...
	if (sysfile_load(rom_name, drive_rom1001, DRIVE_ROM1001_SIZE, DRIVE_ROM1001_SIZE) < 0)
	{
		#ifdef CELL_DEBUG
		printf("ERROR: 1001 ROM image not found.\nHardware-level 1001/8050/8250 emulation is not available.\n");
		#endif
	}
	else
	{
	...
	if (rom_loaded)
	{
		switch (drive->type)
		{
			case DRIVE_TYPE_2031:
				memcpy(&(drive->rom[0x4000]), drive_rom2031, DRIVE_ROM2031_SIZE);
				break;
			case DRIVE_TYPE_2040:
				memcpy(&(drive->rom[DRIVE_ROM_SIZE - DRIVE_ROM2040_SIZE]), drive_rom2040, DRIVE_ROM2040_SIZE);
				break;
			case DRIVE_TYPE_3040:
				memcpy(&(drive->rom[DRIVE_ROM_SIZE - DRIVE_ROM3040_SIZE]), drive_rom3040, DRIVE_ROM3040_SIZE);
				break;
			case DRIVE_TYPE_4040:
				memcpy(&(drive->rom[DRIVE_ROM_SIZE - DRIVE_ROM4040_SIZE]), drive_rom4040, DRIVE_ROM4040_SIZE);
				break;
			case DRIVE_TYPE_1001:
			case DRIVE_TYPE_8050:
			case DRIVE_TYPE_8250:
				memcpy(&(drive->rom[0x4000]), drive_rom1001, DRIVE_ROM1001_SIZE);
				break;
		}
	}
}
...
int ieeerom_check_loaded(unsigned int type)
{
	switch (type)
	{
		...
		case DRIVE_TYPE_8250:
			...
			break;
		case DRIVE_TYPE_ANY:
			if ((!rom2031_loaded && !rom2040_loaded && !rom3040_loaded && !rom4040_loaded && !rom1001_loaded) && rom_loaded)
				return -1;
			break;
		default:
			return -1;
